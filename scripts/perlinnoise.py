...
class PerlinNoise:
    ...
    def randomGrid(self, ix, iy):
        ...
        random = 2920.0 * sin(ix * 21942.0 + iy * 171324.0 + 8912.0 + self.seed) * cos(
            ix * 23157.0 * self.seed * iy * 217832.0 + 9758.0)
    ...
    def find_empty_xy(self, blockw):
        ...
        for i in range(1, int(len(self.map))):
            for k in range(1, i + 1):
                if self.map[y][x] != 2 and self.map[y + 1][x] == 2:
                    # check if emplacement is clear (replace by 0 in next update)
